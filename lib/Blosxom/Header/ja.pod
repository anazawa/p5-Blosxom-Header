...
Blosxom::Header インスタンスは、メモリ空間内に１つしか作成されない。
...
状況に応じて、２つのインタフェースを使い分けることができる。
L<HTTP::Headers> との違いは、１つのフィールドに１つの値しか割り当てられない
...
出力してしまう。Content-Type ヘッダを出力したくない場合、空の文字列を代入
しなければならない。
